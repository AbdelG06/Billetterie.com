...
--
-- Hôte : 127.0.0.1
-- Généré le : mer. 06 mai 2026 à 15:04
...
CREATE TABLE `achats_details` (
  `id_achat_detail` int(11) NOT NULL,
  ...
  `type_tarif` enum('tarif reduit','normale','vip') NOT NULL,
  `quantite` int(11) NOT NULL DEFAULT 1,
  `mode_paiement` enum('PayPal','Carte Bancaire') NOT NULL,
...
--
-- Doublure de structure pour la vue `vue_details_ventes`
-- (Voir ci-dessous la vue réelle)
--
CREATE TABLE `vue_details_ventes` (
`id_commande` int(11)
,`date_commande` timestamp
,`statut_client` varchar(19)
,`nom_acheteur` varchar(100)
,`prenom_acheteur` varchar(100)
,`email_contact` varchar(100)
,`nom_evenement` varchar(150)
,`categorie_billet` varchar(50)
,`niveau_tarif` enum('tarif reduit','normale','vip')
,`quantite` int(11)
,`mode_paiement` enum('PayPal','Carte Bancaire')
...
--
-- Structure de la vue `vue_details_ventes`
--
DROP TABLE IF EXISTS `vue_details_ventes`;

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `vue_details_ventes`  AS SELECT `c`.`id_commande` AS `id_commande`, `c`.`date_commande` AS `date_commande`, CASE WHEN `ad`.`id_utilisateur` is not null THEN 'Utilisateur Inscrit' ELSE 'Simple Visiteur' END AS `statut_client`, `ad`.`nom` AS `nom_acheteur`, `ad`.`prenom` AS `prenom_acheteur`, `ad`.`email` AS `email_contact`, `e`.`titre` AS `nom_evenement`, `tt`.`libelle_tarif` AS `categorie_billet`, `ad`.`type_tarif` AS `niveau_tarif`, `ad`.`quantite` AS `quantite`, `ad`.`mode_paiement` AS `mode_paiement`, `c`.`montant_total` AS `montant_total`, `c`.`statut_paiement` AS `statut_paiement` FROM ((((`commandes` `c` join `achats_details` `ad` on(`c`.`id_commande` = `ad`.`id_commande`)) join `billets_generes` `bg` on(`c`.`id_commande` = `bg`.`id_commande`)) join `tickets_types` `tt` on(`bg`.`id_ticket_type` = `tt`.`id_ticket_type`)) join `evenements` `e` on(`tt`.`id_evenement` = `e`.`id_evenement`)) ;
...
ALTER TABLE `achats_details`
  ADD PRIMARY KEY (`id_achat_detail`),
  ADD KEY `fk_detail_commande` (`id_commande`),
  ADD KEY `fk_detail_utilisateur` (`id_utilisateur`);
...
--
ALTER TABLE `achats_details`
  MODIFY `id_achat_detail` int(11) NOT NULL AUTO_INCREMENT;
...
--
ALTER TABLE `achats_details`
  ADD CONSTRAINT `fk_detail_commande` FOREIGN KEY (`id_commande`) REFERENCES `commandes` (`id_commande`) ON DELETE CASCADE,
  ADD CONSTRAINT `fk_detail_utilisateur` FOREIGN KEY (`id_utilisateur`) REFERENCES `utilisateurs` (`id_utilisateur`) ON DELETE SET NULL;
